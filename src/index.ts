import { EngineObject, vec2, initTileCollision, tileCollisionSize, setTileCollisionData, setCameraScale, setCameraPos,
  mainCanvas, setGravity, mouseWasPressed, mouseIsDown, randColor, mousePos, randInCircle, Color, cameraScale, clamp, rand, mouseWheel, lerp, engineObjects,
  gravity, sign, glDraw, engineInit, Music, max
} from '@knopkem/little-game-engine-ts';

let sprites: any, timeMS: any, showFPS: any, statsDisplay: any, spriteColor: any, spriteAdditiveColor: any, musicSource: any;
...
function gameInit()
{
    ...
    mainCanvas.setAttribute('style', 'background: #555');
    sprites = [];
    setGravity(-.01);
    ...
    statsDisplay.setAttribute('style', 'position:absolute;width:100%;top:50%;left:50%;transform:translate(-50%,-50%);font-size:4em;text-align:center;font-family:arial;user-select:none');
}
...
function gameUpdate()
{
    ...
    // mouse wheel = zoom
    setCameraScale( clamp(cameraScale*(1-mouseWheel/10), 1e3, 1) );
}
